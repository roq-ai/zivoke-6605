...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'PUT':
      return updateTimelineById();
    case 'DELETE':
      return deleteTimelineById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getTimelineById() {
    const data = await prisma.timeline.findFirst(convertQueryToPrismaUtil(req.query, 'timeline'));
    return res.status(200).json(data);
  }

  async function updateTimelineById() {
  ...
  async function deleteTimelineById() {
